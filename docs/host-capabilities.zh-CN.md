...
状态：2026-09-03 已在 EdgeEver 本地开发代码实现，尚未发布。0.4 插件候选需要支持 `ai:generate` 和 `network:public` 的 EdgeEver 版本。普通用户通过正常升级 EdgeEver、安装插件即可使用，不需要源码补丁或额外研究服务。
...
| 研究状态、追踪对比、默认设置声明 | 已有存储、设置、笔记与调度 API |
...
宿主正式发布和生产 Web／桌面验证仍是发布条件。本分支不承诺兼容旧生产版本，不为当前候选创建发布标签。
