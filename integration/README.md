...
Review the SDK permissions/types, client bridge, web plugin context/WorkspaceApp wiring, shared API registration, fixed source adapters, dependency/lockfile changes, the reactive mount fix in PluginPanelDialog, serialized asynchronous startup/cleanup, and lookup of synchronized temporary note IDs. Add the documented APIs to the bilingual plugin-development guide. Follow EdgeEver's normal main-branch and release procedures when shipping.
